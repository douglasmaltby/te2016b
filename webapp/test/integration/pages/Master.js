sap.ui.define([
	...
	"com/test/teched/te2016b/test/integration/pages/Common",
		...
		onTheMasterPage : {
			...
			actions : {
				...
				iSortTheListOnName : function () {
					return this.iChooseASorter("sortButton", "Sort By <Company>");
				},
				...
				iRememberAnIdOfAnObjectThatsNotInTheList : function () {
					return this.waitFor(this.createAWaitForAnEntitySet({
						entitySet : "BusinessPartnerCollection",
						success : function (aEntityData) {
							this.waitFor({
								...
								success : function (oList) {
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function (oObject) {
											return !oList.getItems().some(function (oListItem) {
												return oListItem.getBindingContext().getProperty("BusinessPartnerKey") === oObject.BusinessPartnerKey;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].BusinessPartnerKey;
									} else {
										sCurrentId = aItemsNotInTheList[0].BusinessPartnerKey;
									}
									...
									oCurrentItem.bindingPath = "/" + oList.getModel().createKey("BusinessPartnerCollection", {
										BusinessPartnerKey : sCurrentId
									});
				...
				iRememberTheListItem : function (oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("BusinessPartnerKey"),
						title: oBindingContext.getProperty("Company")
					};
				}
			},
			...
			assertions : {
				...
				theListHeaderDisplaysZeroHits : function () {
					return this.waitFor({
						...
						id: "masterPageTitle",
						autoWait: false,
						matchers: new PropertyStrictEquals({name : "text", value : "<BusinessPartnerCollection> (0)"}),
						success: function () {
				...
				theListShouldBeSortedAscendingOnName : function () {
					return this.theListShouldBeSortedAscendingOnField("Company");
				},
				...
				theListShouldHaveAllEntries : function () {
					...
					// retrieve all BusinessPartnerCollection to be able to check for the total amount
					...
					return this.waitFor({
						...
						success : function (oList) {
							...
							this.waitFor({
								id : "masterPageTitle",
								viewName : sViewName,
								matchers : new PropertyStrictEquals({name : "text", value : "<BusinessPartnerCollection> (" + iExpectedLength + ")"}),
								success : function () {
